...
public class SaveData {
    ...
    //ワールドクラス
    public SaveDataOfWrold saveDataOfWorld;
    //インベントリ
    ...
    public SaveData(Main main, Database database) {
        //現在いる部屋
        currentRoomID = database.GetRoomID(main.currentRoom.data);
        //ワールドクラス
        saveDataOfWorld = new SaveDataOfWrold(main.world, main.database);
        //インベントリ
...
public class Main : MonoBehaviour
{
    ...
    gmState priviousState = gmState.System_Title;
    gmState currentState = gmState.System_Title;
    ...
    private void Initialize() {
        ...
        //ステート初期化
        StartEvent(database.eventDatabase[0]);
    ...
    void Load(SaveData saveData) {
        ...
        //worldクラスのロード
        world = new World(saveData.saveDataOfWorld, database);
        currentRoom = world.GetRoom(database.roomDatabase[saveData.currentRoomID]);
        //Inventoryクラスのロード
    ...
    private void Update() {
        ...
        switch (currentState) {
            ...
            case gmState.System_Title: {
                    if (stateEnter) {
                        stateEnter = false;
                        systemUI.panel_Title.SetActive(true);
                        AudioManager.instance.Play_Music(null);
                    }

                    if (systemUI.pb_Retry) {
                        Initialize();
                        systemUI.panel_Title.SetActive(false);
                        return;
                    }
                            ...
                            if(currentEvent.nextEvent != null) {
                                ...
                                if(currentRoom.enemy != null) {
                                    ...
                                    if (currentEvent.isFinal) {
                                        ChangeState(gmState.System_Ending);
                                        return;
                                    }
            ...
            case gmState.System_GameOver: {
                    if (stateEnter) {
                        ...
                        systemUI.text_GameOver.text = "GameOver";
                        AudioManager.instance.Play_Music(null, 0f);
                    ...
                    if (systemUI.pb_Back) {
                        ChangeState(gmState.System_Title);
                        systemUI.panel_GameOver.SetActive(false);
                        return;
                    }
            ...
            case gmState.System_Ending: {
                    if (stateEnter) {
                        stateEnter = false;
                        systemUI.panel_GameOver.SetActive(true);
                        systemUI.text_GameOver.text = "HappyEnd!!\nThank you for Playing!!!";
                        AudioManager.instance.Play_Music(AudioManager.instance.bgm_Ending);
                    }
