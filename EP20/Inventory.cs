...
public class Inventory
{
    ...
    public Inventory(Main _main,SaveDataOfItem[] itemSaveDataArray,int equipItemIndex , DataBase database) {
        Debug.Log("Inventory初期化開始");
        main = _main;
        Debug.Log("mainを代入");

        itemList = new List<Item>();
        Debug.Log("itemListを初期化");
        foreach (SaveDataOfItem itemSaveData in itemSaveDataArray) {
            Debug.Log("アイテムを生成ID:"+itemSaveData.ID);
            itemList.Add(new Item(itemSaveData, database));
        }

        if(equipItemIndex != -1) {
            equip = itemList[equipItemIndex];
        }
    }
}
